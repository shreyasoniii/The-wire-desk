 ...
 */
export function betaUsersListWorkspaces(client, security, request, options) {
    return new APIPromise($do(client, security, request, options));
}
async function $do(client, security, request, options) {
    const parsed = safeParse(request, (value) => operations.UsersApiListWorkspacesRequest$outboundSchema.optional().parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result, raw] = await M.match(M.json(200, operations.UsersApiListWorkspacesResponse$inboundSchema, {
        key: "Result",
    ...
    const nextFunc = (responseData) => {
        ...
        const nextVal = () => betaUsersListWorkspaces(client, security, {
            ...request,
            offset: nextOffset,
        }, options);
...
//# sourceMappingURL=betaUsersListWorkspaces.js.map