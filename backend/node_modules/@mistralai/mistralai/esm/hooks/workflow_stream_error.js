...
const DEFAULT_REASON = "stream_error";
...
function parseErrorPayload(data) {
    ...
    return {
        error,
        reason: VALID_REASONS.includes(reason)
            ? reason
            : DEFAULT_REASON,
    };
}
function errorFromFrame(block) {
...
class FrameScanner {
    ...
    flush() {
        if (this.error || !this.buffer.length)
            return [];
...
function createErrorDetectingResponse(response) {
...
export class WorkflowStreamErrorHook {
...
//# sourceMappingURL=workflow_stream_error.js.map